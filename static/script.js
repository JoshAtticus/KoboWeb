// KoboWeb JavaScript - Compatible with Android Browser 4 / Android 2.0
// Using only ES3/ES5 features for maximum compatibility

console.log('KoboWeb script loaded - v5');
...
var AccountManager = {
    ...
    getActiveAccount: function() {
        try {
            var activeId = localStorage.getItem('activeAccountId');
    ...
    saveAccount: function(service, username, token) {
        try {
            ...
            if (!localStorage.getItem('activeAccountId')) {
    ...
    removeAccount: function(accountId) {
        try {
            ...
            if (localStorage.getItem('activeAccountId') === accountId) {
    ...
    migrateOldData: function() {
        try {
            var oldToken = localStorage.getItem('wotoken');
            var oldUsername = localStorage.getItem('wousername');
            ...
            if (oldToken && oldUsername) {
                ...
                // Clean up old data
                localStorage.removeItem('wotoken');
                localStorage.removeItem('wousername');
            }
        } catch (e) {
...
var EmailManager = {
    getAllEmails: function() {
        try {
            var emailsJson = localStorage.getItem('emailAccounts');
    ...
    getActiveEmail: function() {
        try {
            var activeId = localStorage.getItem('activeEmailId');
            if (!activeId) return null;
    ...
    saveEmail: function(email, password, server, port, useSSL) {
        try {
            ...
            localStorage.setItem('emailAccounts', JSON.stringify(emails));
            
            if (!localStorage.getItem('activeEmailId')) {
    ...
    removeEmail: function(emailId) {
        try {
            ...
            localStorage.setItem('emailAccounts', JSON.stringify(filtered));
            
            if (localStorage.getItem('activeEmailId') === emailId) {
                localStorage.removeItem('activeEmailId');
                if (filtered.length > 0) {
    ...
    setActiveEmail: function(emailId) {
        try {
            localStorage.setItem('activeEmailId', emailId);
            return true;
        } catch (e) {
